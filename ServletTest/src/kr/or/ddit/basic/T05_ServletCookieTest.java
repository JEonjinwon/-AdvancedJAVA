 ...
 *     - 값
 *     - 유효시간(초)
 *      - 도메인 : ex) www.somehost.com, .somehost.com => 쿠키의 도메인이 쿠키를 생성한 서버의 도메인을 벗어나면 브라우저는 쿠키를 저장(생성)하지 않는다.
 *                                       => 보안 문제 때문에....
 *  2. 동작방식
 *     - 쿠키 생성단계 : 생성한 쿠키를 응답데이터의 헤더에 저장하여 웹브라우저에 전송한다.
 *     - 쿠키 저장단계 : 웹브라우저는 응답데이터에 포함된 쿠키를 쿠키저장소에 보관한다.
 *              (쿠키 종류에 따라 메모리나 파일에 저장함.)
 *     - 쿠키 전송단계 : 웹브라우저는 지정한 쿠키를 요청이 있을때마다 웹서버에 전송한다.
 *              (삭제되기 전까지....)
...
public class T05_ServletCookieTest  extends HttpServlet{
   ...
   protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      //readCookieExam(req,resp);   //쿠키 정보 예제
      
      deleteCookieExam(req,resp);   //쿠키 정보 삭제
   ...
   */
    ...
    * 
    * 2. 쿠키 최대 지속시간을 설정한다.(초단위) => 지정하지 않으면 브라우저를 종료할때 쿠키를 함께 삭제한다.
    ...
    */
      ...
      //쿠키 소멸 시간 설정(초단위) => 지정하지 않으면 웹브라우저를 종료할때 쿠키를 함께 삭제한다.
